<?php /* Template Name: Homepage */ get_header(); ?>

<section class="bannerSection">
    <div class="wrapper">
        <div class="bannerContent">
            <h1 class="roboto-mono">Your Enterprise Truth Amplified</h1>
            <p>We take what already makes your enterprise work and scale it, with AI-native talent, embedded engineers, and outcome-owned enterprise solutions. Built for enterprises and the GCCs that run them.</p>
            <a href="#" class="primaryWhiteCTA roboto-mono"><span>See The Platform</span>
                <div class="peelBox"></div>
                <div class="hoverBox"></div>
            </a>
        </div>
        <img src="<?php bloginfo('template_directory');?>/images/bannerbg.png" alt="">
    </div>
</section>

<section class="enterpriseSection">
    <div class="wrapper">
        <div class="globeEffect">
            <img src="<?php bloginfo('template_directory');?>/images/globe.png" alt="">
        </div>
        <div class="enterpriseContent">
            <h2 class="roboto-mono">The idea behind Enterprise Truth</h2>
            <p>Every great organization has its own recipe for decisions. That is the real source of advantage. We call it your Enterprise Truth, and it cannot be copied. It can only be grown</p>
        </div>
    </div>
</section>

<section class="outcomeSection">
    <div class="secWrapper">
        <div class="secHeading ">
            <div class="headingGroup">
                <h2 class="roboto-mono">Outcomes, Not Promises
                    <svg xmlns="http://www.w3.org/2000/svg" width="12" height="16" viewBox="0 0 12 16" fill="none">
						<path d="M0 15.7028V11.0988H3.69961V8.30356H0V3.78182H3.69961V0H8.30356V3.78182H12.0032V8.30356H8.30356V11.0988H12.0032V15.7028H7.48143V11.921H4.52174V15.7028H0Z" fill="#00ACB4"/>
					</svg>
                </h2>
                <p class="Satoshi-Regular">A few representative outcomes. Real, anonymized, and measured against a baseline.</p>
            </div>
            <a href="#" class="roboto-mono secondaryWhiteCTA"><span>See All Case Studies</span>
                <div class="hoverBox"></div>
            </a>
        </div>
        <div class="boxLayout">
            <div class="box">
                <img src="<?php bloginfo('template_directory'); ?>/images/outcome-card-1.png" alt="">
                <div class="contentBox">
                    <h5 class="roboto-mono">GCC BUILD, APAC BANK</h5>
                    <div class="bottomContent">
                        <h4 class="Satoshi-Regular">$1.2B annual value</h4>
                        <p class="Satoshi-Regular">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Voluptatum quae quas velit? Sed quam eaque sequi quas animi tempora.</p>
                    </div>
                </div>
            </div>
            <div class="box">
                <img src="<?php bloginfo('template_directory'); ?>/images/outcome-card-1.png" alt="">
                <div class="contentBox">
                    <h5 class="roboto-mono">AI DATA & BFSI</h5>
                    <div class="bottomContent">
                        <h4 class="Satoshi-Regular">$2.3M annual OPEX saved</h4>
                        <p class="Satoshi-Regular">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Voluptatum quae quas velit? Sed quam eaque sequi quas animi tempora.</p>
                    </div>
                </div>
            </div>
            <div class="box">
                <img src="<?php bloginfo('template_directory'); ?>/images/outcome-card-1.png" alt="">
                <div class="contentBox">
                    <h5 class="roboto-mono">AI & DATA, RENEWABLE ENERGY</h5>
                    <div class="bottomContent">
                        <h4 class="Satoshi-Regular">70-80% less manual inspection</h4>
                        <p class="Satoshi-Regular">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Voluptatum quae quas velit? Sed quam eaque sequi quas animi tempora.</p>
                    </div>
                </div>
            </div>
            <div class="box">
                <img src="<?php bloginfo('template_directory'); ?>/images/outcome-card-1.png" alt="">
                <div class="contentBox">
                    <h5 class="roboto-mono">TRANSFORMATION, INDIAN BANK</h5>
                    <div class="bottomContent">
                        <h4 class="Satoshi-Regular">42% turnaround-time cut</h4>
                        <p class="Satoshi-Regular">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Voluptatum quae quas velit? Sed quam eaque sequi quas animi tempora.</p>
                    </div>
                </div>
            </div>
            <img src="<?php bloginfo('template_directory'); ?>/images/star-icon.png" alt="" class="starIcon">
        </div>
    </div>
</section>

<?php get_footer(); ?>